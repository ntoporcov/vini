...
actor ServiceController {
    ...
    func start(_ service: ViniService) async throws {
        switch service.kind {
        ...
        case .userDefined(let def):
            // Vini owns the process so it can report status and stop it later.
    ...
    func stop(_ service: ViniService) async throws {
        switch service.kind {
        case .homebrew(let formula):
    ...
    func restart(_ service: ViniService) async throws {
        switch service.kind {
        case .homebrew(let formula):
...
enum ServiceControlError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .notControllable(let name):
            "'\(name)' was detected by port only and can't be started or stopped by Vini."
