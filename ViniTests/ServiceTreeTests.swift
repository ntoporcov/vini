...
final class ServiceTreeTests: XCTestCase {
    ...
    private func svc(_ id: String, _ name: String) -> ViniService {
        ViniService(id: id, name: name, kind: .homebrew(formula: name),
    ...
    func testTopLevelGroupsPreserveStoredOrder() {
        let b = ServiceGroup(name: "B")
        let a = ServiceGroup(name: "A")
        let tree = ServiceTree.build(groups: [b, a], services: [])
        XCTAssertEqual(tree.map(\.name), ["B", "A"])
    }

    func testUngroupedServicesUseManualOrderFirst() {
        let redis = svc("brew:redis", "Redis")
        let pg = svc("brew:pg", "PostgreSQL")
        let tree = ServiceTree.build(groups: [], services: [pg, redis], serviceOrderIDs: ["brew:redis", "brew:pg"])
        XCTAssertEqual(tree.first?.children.map(\.name), ["Redis", "PostgreSQL"])
    }
